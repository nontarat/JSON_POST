...
err_t tcp_write_time_msg(struct tcp_pcb *pcb, const char *format) {
  ...
  const char *json_data = "{\"temperature\": 70, \"humidity\": 50}"; // ปรับปรุง JSON data ให้ถูกต้อง
	...
	//  MessageLen = snprintf((char*)Message, sizeof(Message), "POST /iot_project/connect.php HTTP/1.1\r\nHost:tanakulgroup.com\r\nContent-Length:50\r\nContent-Type:application/json\r\n\r\n%s",json_data);
	    MessageLen = snprintf((char*)Message, sizeof(Message), "POST /iot_project/connect_json_post.php HTTP/1.1\r\nHost:tanakulgroup.com\r\nContent-Type:application/json\r\nContent-Length:%d\r\n\r\n%s", strlen(json_data), json_data);
